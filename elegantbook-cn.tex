...
\begin{remark}
如果你想把 \lstinline{problemset} 环境的标题改为其他文字，你可以类似于 introduction 环境修改 problemset 的可选参数。另外，这个环境会自动出现在目录和页眉信息中（版本4.1）。
\end{remark}
...
\datechange{2026/05/01}{版本 4.7 正式发布。}

\begin{change}
  \item 修复：双面模式（twoside）下附录章节的页眉显示错误（\href{https://github.com/ElegantLaTeX/ElegantBook/issues/256}{issue: \#256}）。
  \item 修复：页面排满/浮动体 [p] 时，页眉提前错误显示下一节标题（\href{https://github.com/ElegantLaTeX/ElegantBook/issues/257}{issue: \#257}）。
\end{change}
